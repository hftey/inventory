...
//            $url = "https://api.fixer.io/latest?base=".$TargetCurrency."&symbols=".$systemSetting->currency;
            ...
            $url = "http://data.fixer.io/api/latest?access_key=017ff063ab6433fdf404b329cfdb24c1&symbols=EUR,USD,AUD,CAD,PLN,MXN,MYR,SGD,GBP,CNY,JPY&format=1";

            $exchange = json_decode(file_get_contents($url));
            $sysCurrency = $systemSetting->currency;

            $exchange = $exchange->rates;
            $default_base_currency = $exchange->EUR; //DEFAULT BASE CURRENCY
            $new_base_currency = $exchange->$TargetCurrency; //NEW BASE CURRENCY
            $base_exchange = $default_base_currency/$new_base_currency;
            foreach($exchange as $key=>$value) {
                $new_exhange_rates[$key] = ($value * $base_exchange);
            }
            print $new_exhange_rates[$sysCurrency];
